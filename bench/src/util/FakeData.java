...
public class FakeData {
   ...
   * @param noise rms of noise (relative to signal) added to the image.
   */
  public static float[][][] seismicAndSlopes2d2014A(double noise, boolean rand){
    int n1 = 501;
    int n2 = 501;
    float[][][] p = makeReflectivityWithNormals(n1,n2);
    float[][][] q = makeReflectivityWithNormals(n1,n2);
  ...
  private static float[][] addNoise(double nrms, float[][] f, boolean rand) {
    int n1 = f[0].length;
    int n2 = f.length;
    Random r;
    if (rand) {
  ...
  private static class PlanarFault3 implements T3 {
    ...
    public PlanarFault3(
      ...
      // Fault normal vector.
      float[] w = FaultGeometry.faultNormalVectorFromStrikeAndDip(fphi,ftheta);
      _w1 = w[0];
      _w2 = w[1];
      _w3 = w[2];
