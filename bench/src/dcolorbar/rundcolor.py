...
user = "git/ea/bench/src/dcolorbar/data/" #elias
#user = "box/cg/bench/src/dcolorbar/data/" #chris
path = System.getProperty("user.home")+"/Home/"+user
def main(args):
  dcolor()
...
def dcolor():
  ...
  s1 = Sampling(n1)
  s2 = Sampling(n2)
  seis = Utility.read(n1,n2,path+"tp73.dat")
  colorBarNames = ["gray.bin","gray_yellow_red.bin","red_white_blue.bin"]
  ...
  #read in image
  imageFile = "daniel_marmousi.png"
  #imageFile = "red_white_blue.png"
  #imageFile = "red_white_blue_crop.png"
  #imageFile = "gray_yellow_red_crop.png"
  #imageFile = "prism_crop.png"
  ...
  optcb = colorBarPacked[minInx]
  nd1 = len(imageRGBValues[0][0])
  nd2 = len(imageRGBValues[0])
  sd1 = Sampling(nd1)
  sd2 = Sampling(nd2)
  rI2 = imageRGBValues[0]
  gI2 = imageRGBValues[1]
  bI2 = imageRGBValues[2]

  slope = zerofloat(nd2,nd1)
  data = Utility.imageToData(optcb,rI2,gI2,bI2)
  sigma1 = 20.0
  sigma2 = 4.0
  lsf = LocalSlopeFinder(sigma1,sigma2,2)
  lsf.findSlopes(data,slope);
  #Utility.plot(sd2,sd1,data)
  #Utility.plot(sd2,sd1,slope)
...
def findSumDistance(rCB,gCB,bCB,rI,gI,bI):
  ...
  for iI in range(0,nI):
    for iCB in range(0,nCB):
      sumDistance += findDistance(rCB[iCB],gCB[iCB],bCB[iCB],
                                  rI[iI],gI[iI],bI[iI])
  return sumDistance
